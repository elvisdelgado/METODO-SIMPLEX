...
numero_varZ = (int(input("Digite el numero de variables de decision que tiene el problema : ")))
numero_inec = (int(input("Digite el numero de restricones: ")))
...
for i in range(num_filas):
    for j in range(num_colum):
        if j == 0 and i != num_filas - 1:
            ...
            matriz_1[i][j] = int(
                input("Digite el coeficiente de la variable " + str(j) + " de la resticcion " + str(i + 1) + ": "))
        elif j == num_colum - 1 and i != num_filas - 1:
            matriz_1[i][j] = int(input("Digite el coeficiente al que esta igualado la restriccion " + str(i + 1) + ": "))
        elif 0 < j <= numero_varZ and i == num_filas - 1:
...
#Consultar salida
continuar = input("\n¿Desea Salir? s = Si ")
while(continuar != "s"):  #Validar salida
    print("Opcion no valida")
    continuar = input("\n¿Desea Salir? s = Si o n = No: ") 
else: 
    if  continuar == "s" :  #Verificar salida
        print("Adios\n")
